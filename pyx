...
if len(sys.argv) == 1:
	...
	sys.stdout.write("\x1b[8;60;200t")
	...
	while 1:
		...
		while rc != dbqry.num_rows():
			...
			try:
				...
				rc += 1
		# Last 40
		qrystr = "select event.timestamp,signature.sig_name,iphdr.ip_src,iphdr.ip_dst from event left join (signature,iphdr) on (event.signature = signature.sig_id and event.sid = iphdr.sid and event.cid = iphdr.cid) where event.timestamp >= Date_Sub(NOW(), Interval 1 hour) order by event.timestamp desc limit 30" 
		dbs.query(qrystr)
		dbqry=dbs.store_result()
		rcnt = []
		rc = 0
		while rc != dbqry.num_rows():
			...
			try:
				...
				rc += 1
		# Top events
		qrystr = "select distinct sig_name,count(cid) from event join signature on signature = sig_id where timestamp >= Date_Sub(NOW(),Interval 1 hour) group by signature order by count(cid) desc limit 30"
		dbs.query(qrystr)
		dbqry=dbs.store_result()
		tope = []
		rc = 0
		while rc != dbqry.num_rows():
			...
			try:
				...
				rc += 1
		# Rare Events
		qrystr = "select t.* from (select distinct sig_name, count(cid) as c from event join signature on signature = sig_id where timestamp >= Date_Sub(NOW(),Interval 1 hour) group by signature order by event.timestamp ) as t where t.c = '1' limit 30"
		dbs.query(qrystr)
		dbqry=dbs.store_result()
		rrev = []
		rc = 0
		...
		screen.addstr(0,138,'  |  Rare Events')
		x = 0 
		for ln in tope:
			x += 1
		...
		screen.addstr(32,85,'  |  Top Dst x Src')
		screen.addstr(32,115,'  |  Top TCP Src Port')
		screen.addstr(32,135,'  |  Top TCP Dst Port')
		screen.addstr(32,155,'  |  Top UDP Src Port')
		screen.addstr(32,175,'  |  Top UDP Dst Port')
		x = 32
		for ln in ipsrc[:26]:
			x += 1
		...
		for ln in ipdst[:26]:
			x += 1
			_ln = '  |  '+decip(ln[1]).ljust(15)+' x '+str(ln[0]).rjust(6)	
			screen.addstr(x,25,_ln)
		x = 32
		for ln in ipsd[:26]:
			x += 1
			_ln = '  |  '+decip(ln[1]).ljust(15)+' x '+str(ln[0]).rjust(6)
			screen.addstr(x,55,_ln)
		x = 32
		for ln in ipds[:26]:
			x += 1	
			_ln = '  |  '+decip(ln[1]).ljust(15)+' x '+str(ln[0]).rjust(6)
			screen.addstr(x,85,_ln)
		x = 32
		for ln in tcpsrc[:26]: 	
			x += 1
			_ln = '  |  '+ln[1].ljust(5)+' x '+str(ln[0]).rjust(6)
			screen.addstr(x,115,_ln)
		x = 32
		for ln in tcpdst[:26]:
			x += 1
			_ln = '  |  '+ln[1].ljust(5)+' x '+str(ln[0]).rjust(6)
			screen.addstr(x,135,_ln)
		x = 32
		for ln in udpsrc[:26]:
			x += 1
			_ln = '  |  '+ln[1].ljust(5)+' x '+str(ln[0]).rjust(6)
			screen.addstr(x,155,_ln)
		x = 32
		for ln in udpdst[:26]:
			x += 1
			_ln = '  |  '+ln[1].ljust(5)+' x '+str(ln[0]).rjust(6)
			screen.addstr(x,175,_ln)
		screen.refresh()
		# Sleep loop
		for x in range(10):
